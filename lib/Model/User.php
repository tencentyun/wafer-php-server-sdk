...
class User
{
    ...
    public function storeUserInfo($userinfo, $skey, $session_key)
    {
        ...
        if ($res === NULL) {
            DB::insert('cSessionInfo', compact('uuid', 'skey', 'create_time', 'last_visit_time', 'open_id', 'session_key', 'user_info', 'app_id'));
        } else {
            DB::update(
                'cSessionInfo',
    ...
    public function findUserBySKey ($skey) {
        $app_id = $this->appInfo->id;
        return DB::row('cSessionInfo', ['*'], compact('app_id','skey'));
    }
}
